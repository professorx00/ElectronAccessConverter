...
async function writeNewFile(AOP) {
  ...
  completedFileName =`./Completed/ProcessedFile-${moment().format(`MM-DD-YYYY`)}.csv`
  writer.pipe(fs.createWriteStream(completedFileName))
  Object.keys(AOP).forEach(employee => {
    ...
    Object.keys(AOP[employee]).forEach(key => {
      ...
      ipc.send('completed', completedFileName)
    })
  })
  writer.end()
}
  ...
  try {
    await processOldFiles(tbrFile, trFile)
      .then(async () => {
        ...
        ipc.on('completed',function(e,item){
          mainWindow.webContents.send('completed',completedFileName)
          addWindow.close();
        })
